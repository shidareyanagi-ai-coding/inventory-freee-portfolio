...
>
> 🛠 **A-2（Neon移行）コード完了 / 実Neon接続は保留**: DBアクセス層を [`db.py`](db.py) に分離し、`DATABASE_URL` で **SQLite ⇄ Postgres（Neon / psycopg3）** を切替えられるようにしました。業務ロジックは方言を意識せず常に `?` で SQL を書き、方言差（`?`/`%s`・`lastrowid`/`RETURNING`・`strftime`/`EXTRACT`・DDL）は `db.py` が吸収します。使い捨てDocker Postgresで検証済みですが、**実際の Neon DB 作成・接続はこれから**（A-2を本当に閉じる残タスク）。
