...
>
> 🛠 **A-2（Neon移行）完了**: DBアクセス層を [`db.py`](db.py) に分離し、`DATABASE_URL` で **SQLite ⇄ Postgres（Neon / psycopg3）** を切替えられるようにしました。業務ロジックは方言を意識せず常に `?` で SQL を書き、方言差（`?`/`%s`・`lastrowid`/`RETURNING`・`strftime`/`EXTRACT`・DDL）は `db.py` が吸収します。**実 Neon（PostgreSQL 16 / Singapore）へ接続し、スキーマ作成・seed投入・全テスト41 passed・ダッシュボード表示まで検証済み**。
>
> 🛠 **A-3（認証＋テナント＋RBAC）コード完了・ローカル検証済**: [Clerk](https://clerk.com/) の JWT を JWKS(RS256) で検証（[`auth.py`](auth.py)）、全ドメインテーブルを `organization_id` で分離、軽量RBAC（admin/staff/viewer）、IDOR封鎖（別テナントの id は 404）、監査ログ、初回ログインで自組織サンドボックスにデモ seed。**実 Clerk キーと Neon テスト用ブランチは未接続**（最終検証はそれらが揃ってから）。
...
全 API は認証必須です。動かし方は 2 通り。

```powershell
# (A) Clerk 無しでローカル開発（dev モード）
#   Authorization 無しのリクエストを既定の dev ユーザ=1組織として扱う。
$env:AUTH_DEV_MODE = "true"
python app.py

# (B) Clerk を使う（本番同等）
#   公開キー/JWKS をサーバに渡すと、フロントがサインインゲートを表示し
#   Authorization: Bearer のトークンを FastAPI が JWKS で検証する。
$env:CLERK_PUBLISHABLE_KEY = "pk_test_xxx"
$env:CLERK_ISSUER = "https://xxxx.clerk.accounts.dev"   # JWKS_URL は未設定なら自動導出
python app.py
```

> dev モードは `APP_ENV=production` のとき強制的に無効化されます（本番で認証を素通りさせない）。
> 秘密キー（`CLERK_SECRET_KEY`）はサーバ側のみ。公開キー（`pk_...`）だけがブラウザに渡ります。
...
python app.py   # 起動時にスキーマDDLを流す
```

起動時に行うのは**スキーマDDLの適用まで**です（A-3）。デモデータの seed は「初回ログインで自組織サンドボックスに」投入されるため、ユーザごとに独立したデータになります。
...
```powershell
python -m pytest -q        # 既定（SQLite）: 45 passed, 14 skipped
```

- `test_app.py`: 業務ロジック関数を直接呼ぶテスト（在庫整合・freeeキュー・予測など / SQLite）。
- `test_api.py`: FastAPI の `TestClient` 経由でルーティング・ステータスコード・`{"error": ...}` 整形を検証（SQLite / dev モード）。
- `test_auth_tenancy.py`: **A-3 の認証ガード(401)・テナント分離=IDOR(404)・RBAC(403)・JWT署名検証** を検証（SQLite / 自己署名RSA）。
- `test_postgres.py`: **Postgres バックエンドのスモークテスト**。`DATABASE_URL` が postgres を指し、かつ `PYTEST_ALLOW_DB_RESET=1` を明示したときだけ実行（本番Neon誤爆防止。未設定ならスキップ）。

SQLite 側のテストは `DATABASE_URL` が設定された環境でも必ずローカル SQLite を使うよう隔離しています。Postgres 側は throwaway コンテナで検証できます。

```powershell
# Postgres も含めて検証する例（要 Docker。本番Neonには絶対に向けない）
...
python -m pytest -q        # 59 passed（SQLite 45 + Postgres 14）
docker rm -f a3pg
```
