  ...
  <script>
    ...
    const MODEL_INFO = {
      baseline: {
        ...
        plain: "「最近どれくらい売れているか」の平均を出し、月ごとの繁忙・閑散のクセで微調整して先を見込む、いちばんシンプルな基準です。",
        method: "<b>移動平均</b>（直近28日をならした平均）＋<b>季節調整</b>（その月が普段より売れる／売れないかの倍率）。凝った仕組みを使わず必ず動くので、他のモデルが本当に賢いかを測る“ものさし”になります。",
        shape: "月内はほぼ<b>水平な一定線</b>（曜日の波は出ません）。突発的なスパイクは平均に薄まるので追わず、外れ値に強く安定します。",
      },
      sarima: {
        label: "SARIMA（古典的な時系列モデル）",
        plain: "「先週の同じ曜日はどうだったか」「最近の増減の流れ」から、その続きを読みます。人が経験で読む“クセ”を数式にした、統計学の定番です。",
        method: "<b>自己回帰</b>（過去の自分の値を手がかりにする）＋<b>差分</b>（増減の傾き＝トレンド）＋<b>移動平均</b>に<b>季節性</b>（曜日など7日周期）を組み合わせた古典的手法「<b>SARIMA</b>」。統計分野で長年使われてきた信頼できる方法です（計算が収束しない時は近い手法ホルト・ウィンターズへ自動で切替）。",
        shape: "<b>なめらかな曲線</b>で、平日が高く週末が低い“週の波”を描きやすいです。",
      },
      lightgbm: {
        label: "LightGBM（機械学習・勾配ブースティング）",
        plain: "「金曜・月末・最近よく売れている…」など多くの条件を組み合わせ、「こういう日は何個売れる」をデータから学習する機械学習です。",
        method: "<b>勾配ブースティング</b>（小さな予測ルール＝決定木を少しずつ継ぎ足して精度を上げる仕組み）。予測コンペ(Kaggle)などで定番の強力な手法です。<b>分位点回帰</b>で「真ん中の予測」に加え「多め／少なめ」の80%の幅も同時に出します。材料は曜日・月（カレンダー）や過去の売上（ラグ）など。",
        shape: "学習した曜日・直近のパターンを反映して<b>ギザギザ・不規則</b>になりやすいです。直近データに敏感で、直近が0続きだと予測も0付近に沈むことがあります（データ量と連続性が効きます）。",
      },
    };
    ...
    function updateModelInfo(modelName) {
      ...
      box.innerHTML =
        ...
        + `<div>・<b>ひとことで</b>：${info.plain}</div>`
        + `<div>・<b>使っている手法</b>：${info.method}</div>`
        + `<div>・<b>グラフの形</b>：${info.shape}</div>`
        + `<div style="color:var(--muted);font-size:12px;margin-top:6px;border-top:1px dashed var(--line);padding-top:6px;">`
        + `共通：3モデルとも、学習には需要履歴の<b>全期間</b>（このデモは約1年）を使います。「直近28日」は精度の答え合わせ（<b>MAE</b>＝予測が実際と平均何個ずれたか）に使う区間<b>だけ</b>で、学習自体は全期間が対象です。予測は30日先まで出し、各商品で<b>MAEが最小だったモデルを★最良として自動採用</b>します。</div>`;
    }
