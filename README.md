...
| 適正在庫シミュレーション | 現在在庫・必要在庫・今すぐ発注量・月末判定を一覧表示（**AI予測ベース**。採用モデルを切り替えると一覧・シミュにも即反映＝探索モード） |
...
| 期末棚卸連携 | 在庫の**期末評価額（Σ 在庫数 × 仕入単価）**を疑似freee の**期末商品棚卸高**へ送信し、**三分法の売上原価（期首＋当期仕入−期末）**とBSの商品残高に反映 |
| 複式簿記・決算（疑似freee） | 取引を**借方/貸方の仕訳に自動展開**し、**試算表・貸借対照表(BS)・損益計算書(PL)・仕訳帳・総勘定元帳**を表示（簿記3級レベル・税込経理）。**三分法**で売上原価（期首商品＋当期仕入−期末商品）を計算 |
