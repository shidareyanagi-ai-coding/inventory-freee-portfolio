"""証憑画像の保存先を抽象化する（A-8 永続化: 疑似freee）。

在庫 `inventory_dashboard/storage.py` を**独立移植**したもの（各アプリは単体で動く方針=NFR-5）。
env の STORAGE_* が揃えば S3 互換オブジェクトストレージ（Cloudflare R2 など）に、無ければ
ローカルフォルダに保存する。呼び出し側（app.py）は「key（相対パス）」だけを扱い、実際の
置き場所はこのモジュールが決める（db.py が DATABASE_URL で SQLite/Postgres を切り替えるのと同じ
「env で切替・薄い手書きアダプタ」方針）。

なぜ必要か（A-8）: Render などのサーバはファイルが再起動で消える（ephemeral）ため、本番では
画像を外部のオブジェクトストレージに置く。ローカル開発・テストは env 未設定なので従来どおり
ローカルフォルダを使う＝開発体験は変わらない。

接頭辞分離（A-8 確定方針）: 在庫と**同じ R2 バケットを流用**するため、オブジェクトストレージ上の
キーには KEY_PREFIX（"pseudo-freee/"）を付けて在庫の画像と分離する。ローカル保存先と DB の
storage_path は従来どおり接頭辞なし（＝保存先がローカルか R2 かに依らず DB の値は同じ）。
...
# 接頭辞分離: R2 を在庫と共有するとき、疑似freee の画像はこの接頭辞配下に置く。
KEY_PREFIX = "pseudo-freee/"


def _env(name: str) -> str:
    """環境変数を読む（前後空白を除去）。`.env` テンプレの行末コメントが値に混入した
    場合に備え '#' 始まりは未設定扱い（auth._env / 在庫 storage._env と同じ防御）。"""
...
def _client():
    ...
    boto3 未導入なら明示エラー（使うときだけ依存を要求する）。"""
    try:
        import boto3
...
def _object_key(key: str) -> str:
    """オブジェクトストレージ上のキー。在庫と同じバケットでも混ざらないよう接頭辞を付ける。"""
...
def save_bytes(local_dir: Path, key: str, data: bytes) -> None:
    ...
    if object_storage_enabled():
        _client().put_object(Bucket=_config()["bucket"], Key=_object_key(key), Body=data)
        return
...
def read_bytes(local_dir: Path, key: str) -> bytes | None:
    ...
    if object_storage_enabled():
        ...
        try:
            obj = _client().get_object(Bucket=_config()["bucket"], Key=_object_key(key))
...
def delete(local_dir: Path, key: str) -> None:
    ...
    if object_storage_enabled():
        try:
            _client().delete_object(Bucket=_config()["bucket"], Key=_object_key(key))
        except Exception:
            pass
        return
    path = Path(local_dir) / key
