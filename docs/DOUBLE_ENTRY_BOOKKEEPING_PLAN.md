...
- **D-4 完了**: 期末棚卸連携（③＝Phase B）。在庫 `stock_by_product` に `as_of`、`closing_inventory_book_amount`（Σ在庫×仕入単価）、`push_closing_inventory`＋ルート `/api/closing-inventory/preview`・`/push`、UIに「📦 決算: 期末在庫を freee へ送る」パネル。疑似freee に `upsert_closing_inventory`＋`POST /api/closing-inventory`（受け皿テーブルは既存）。送信は freee_sync_queue を介さず直接 POST＋record_audit（決済キューを汚さない・period 単位 upsert で冪等）。在庫 60＋疑似freee 65 passed。ローカル実機で 期末在庫 push→疑似freee `/api/statements` の BS `商品`＝送信額（777000）・貸借一致 を確認。
- **D-5 完了**: 突合ビュー（④）。在庫バックエンドが疑似freee `/api/reconciliation`（売上高=income純額・仕入高=purchase純額・商品=期末実地）をサーバ間取得し、在庫の「会計に映すべき総額」（取消除外の active な仕入/売上を build_freee_payload と同じ round で税込集計＋現在の期末在庫評価額）と比較。`reconciliation()`＋`GET /api/reconciliation`、UIに「🔗 会計突合」パネル（項目/在庫/疑似freee/差分/判定・一致✓バッジ・疑似freee停止時は案内）。在庫 61＋疑似freee 66 passed。実機で view 動作・差分検出・期末在庫一致・疑似freee停止時の graceful を確認（クロスアプリのデータドリフトも正しく検出＝累積したdev DBの旧CSV売上などを差分表示）。**全項目グリーンの突合は D-6 のコヒーレントなサンプルデータで実現する**。
- **D-6 ローカル構築 完了（ライブは未／ユーザ合図待ち）**: 既存デモ商品ライン（PC周辺機器5品）でコヒーレントなデモデータを生成。ビルダ＝`scripts/build_sample_data.py`（両DBを1回リセット→商品5・仕入5・売上20→一括送信→期末在庫push→突合検証）と `scripts/build_demand_history.py`（2025-06〜2026-05 の日次需要1,733行を `demand_history` へ）。**ローカル実機で突合オールグリーン**（売上高 1,162,700／仕入高 893,200／期末在庫 240,500 が在庫＝疑似freee で完全一致）＋予測バッチ3モデル（baseline/SARIMA/LightGBM）成功を確認。需要履歴は会計に流れず突合は不変。
  - **残（ライブ反映・ユーザ合図で着手）**: 本番(Render/Neon)へ同じデータを投入。ライブは Clerk 認証必須なので dev-mode 用の上記スクリプトはそのままでは使えない＝認証付きセッション or トークン経由での投入方法をユーザと確定してから実施。投入前にライブ両DBの初期化（1回）。
...
6. **取引先マスタの整合（2026-06-24 ユーザ指摘②）**: 疑似freee の `create_deal` は受領した `partner_name` を deal に文字列保存するだけで `pseudo_freee_payees`（payee マスタ）に登録しない＝在庫 `business_partners` と未連携（仕訳帳には出るがマスタ欄に出ない）。**現状は取引先別集計が無いので数値は正しく問題なし**。だが「取引先別の売掛金/買掛金残高・取引先元帳・売掛金年齢表」を入れる段で、取引先の名寄せ（共有マスタ or マッピング）が必要になる。Phase D の中核。

### 関連する小修正（Phase D とは独立して着手可）
- **在庫の取引先マスタ編集/削除（2026-06-24 ユーザ指摘①）**: 在庫側は `add_business_partner`/`create_business_partner`（追加）・`list_business_partners`（一覧）・`sync_partner_master` のみで **`update`/`delete` が無い**(inventory app.py:544-604)。誤登録（名前の打ち間違え等）を直せない。取引先マスタの編集・削除（or 名寄せ/マージ）導線を足す小改修。past の purchase/sale 行・freee へ送った deal は partner_name を非正規化保持している点に注意（編集時の整合方針も要検討）。

### サンプルデータ整備（Phase D の成果物・2026-06-24 ユーザ承認）
Phase D の連携完成と同時に、**コヒーレントなデモデータ一式を生成する（担当=AI）**。手順: (a) 在庫＋疑似freee を**1回だけ**初期化（今は初期化しない＝二度手間回避） → (b) 在庫に約1ヶ月分の架空の仕入/売上を登録 → (c) 疑似freee へ push＋経費計上して完成。これが「在庫⇄会計が一致している」ことの生きたデモになる。
- **ターゲット = ライブ(Render/Neon)**（来訪者に見せるため。ローカルで作って検証→ライブの順で慎重に）。
- **予測用の長い需要履歴 と 会計用の実取引（直近）を分離**してビルド（=上記スコープ⑤の CSV 分離とセット。予測モデル SARIMA/LightGBM は数ヶ月〜1年の日次履歴が要るため、1ヶ月の手作りだけでは予測デモが弱くなる）。
